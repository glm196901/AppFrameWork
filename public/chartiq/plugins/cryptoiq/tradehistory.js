/* removeIf(umd) */ ;(function(root, factory) {
	...
	TradeHistory.prototype.attachedCallback=function(){
	...
	TradeHistory.prototype.setContext=function(context){
		var self = this;
	...
	TradeHistory.prototype.update=function(last){
	...
	TradeHistory.prototype.clearTable=function(selector){
		var side=this.node.find(selector);
		if(!side.length) return;
		side.find("cq-item").remove();
	};

	TradeHistory.prototype.updateTableRow=function(data, selector, reverseOrder){
			...
			return function(){
				...
				if(myCol && record[myCol]!==undefined){
					...
					if(myCol=="time") val=record[myCol];
						...
						val=Number(record[myCol].toFixed(8)); // remove roundoff error
	...
	TradeHistory.prototype.open=function(){
		this.node.attr("cq-active",true);
	};

	TradeHistory.prototype.close=function(){
		this.node.removeAttr("cq-active");
	};

	CIQ.UI.TradeHistory=document.registerElement("cq-tradehistory", TradeHistory);
